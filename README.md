...
Discord-бот для верификации участников через анкеты, с модерацией заявок, апелляциями и публикацией готовых embed-сообщений (`fluffycore`, `info1`, `info2`, `navigation`, `rules1`, `rules2`, `terms`). Дополнительно автоматически выдаёт роль за тег сервера (Server Tag). Команды — слеш-команды (`/верификация`, `/апелляция` — owner; `/запостить`, `/амнистии`, `/выдатьчсп`, `/снятьчсп` — ststaff; `/анкеты`, `/история`, `/тег` — staff), данные хранятся во внешней **MySQL/MariaDB** (через `mysql2`). Бот **мультисерверный**: один процесс обслуживает все серверы, на которых он есть, а роли и каналы настраиваются для каждого сервера отдельно.
...
│   │   ├── history.ts         /история (staff) — история заявок, апелляций и ЧС участника
...
│   │   ├── historyPage.ts     Пагинация истории участника (history:page)
...
│   ├── schema.ts              Схема таблиц Drizzle ORM (applications, appeals, counters, join_methods, guild_settings, app_config, user_history)
...
4. **Настрой серверы.** Конфигурации в файле нет — настройки каждого сервера лежат в таблице `guild_settings` (по строке на каждый ключ). Шаги ниже повтори для **каждого** сервера, где работает бот.
    ...
    **4.4. Вставь строки в `guild_settings`** через SQL или GUI (phpMyAdmin / Adminer / DBeaver). Ключи — с точками; `roles.staff` и `roles.ststaff` — это списки ID ролей через запятую. Замени плейсхолдеры на свои ID:
    ...
    | `roles.staff` | да | Роли staff через запятую — анкеты и история: `/анкеты`, `/история`, `/тег`, кнопки решения по анкетам |
    ...
    **Уровни доступа** (строгий режим — доступ дают только эти роли и owner-ID, право Discord «Администратор» само по себе ничего не даёт):
    ...
    - **staff** (`roles.staff`) — анкеты и история: `/анкеты`, `/история`, `/тег`, кнопки решения по анкетам.
